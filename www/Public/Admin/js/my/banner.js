...
function BatcDelete(){
  ...
  if (confirm('您确定要删除选中的资质吗？')) {
    $.post("/admin.php/Admin/BatcDeleteBanner",{'ids':chk_value},function(result){
    // console.log(result);
...
function Delete(id){
  if (confirm('您确定要删除这个资质吗？')) {
    $.post("/admin.php/Admin/DeleteBanner",{'id':id},function(result){
      if (result=='y') {
        alert('删除成功！');
        location.reload();
        return;
  ...
  function click_up_img(){
    ...
    function up_image(){
        $("#form_upimg").ajaxSubmit(function(message) { 
            ...
            $("#preview_img").show();
            $("#preview_img").attr("src",'/Uploads/Public/uploads/img/banner/'+message);
