...
function BatcDelete(){
  ...
  if (chk_value.length==0) {alert('请先选择要删除的招聘信息');return;}
  // console.log(chk_value);

  if (confirm('您确定要删除选中的招聘信息吗？')) {
    $.post("/admin.php/Admin/BatcDeleteRecruitment",{'ids':chk_value},function(result){
    // console.log(result);
...
function Delete(id){
  if (confirm('您确定要删除这条招聘信息吗？')) {
    $.post("/admin.php/Admin/DeleteRecruitment",{'id':id},function(result){
      if (result=='y') {
        alert('删除成功！');
        location.reload();
        return;
