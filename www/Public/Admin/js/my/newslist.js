...
function BatcDelete(){
  ...
  if (chk_value.length==0) {alert('请先选择要删除的新闻');return;}
  // console.log(chk_value);

  if (confirm('您确定要删除选中的新闻吗？')) {
    $.post("/admin.php/Admin/BatcDeleteNews",{'ids':chk_value},function(result){
    // console.log(result);
...
function Delete(id){
  if (confirm('您确定要删除这条新闻吗？')) {
    $.post("/admin.php/Admin/DeleteNews",{'id':id},function(result){
      if (result=='y') {
        alert('删除成功！');
        location.reload();
        return;
