...
function BatcDelete(){
  ...
  if (confirm('您确定要删除选中的产品吗？')) {
    $.post("/admin.php/Admin/BatcDeleteProducts",{'ids':chk_value},function(result){
    // console.log(result);
...
function Delete(id){
  if (confirm('您确定要删除这个产品吗？')) {
    $.post("/admin.php/Admin/DeleteProduct",{'id':id},function(result){
      if (result=='y') {
        alert('删除成功！');
        location.reload();
        return;
...
function add_class(){
    $("#form_class").ajaxSubmit(function(message) { 
        console.log(message);
        if(message=='y'){
          alert('添加成功！');
          return;
...
function Delete_class(id){
  if (confirm('您确定要删除这个分类吗？')) {
    $.post("/admin.php/Admin/productclass/action/del",{'id':id},function(result){
      if (result=='y') {
        alert('删除成功！');
        location.reload();
        return;
